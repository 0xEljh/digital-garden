...
At the time, semi-supervised methods were all the rage. Within the year, I also just discovered [distil.pub](https://distill.pub/) (which I miss dearly) and was absolutely possed by [this beautiful article called "Why Momentum really works"](https://distill.pub/2017/momentum/).

As a result, the two biggest training optimisations I pursued were:

1. data augmentation pipelines
2. LR scheduling (reduction on plateau)
...
If we interpret "learning rate" as scale factor for how much the parameters can change, then PyTorch's implementation makes sense.

But if we're trying to recreate the monentum and learning rate mechanics like in the "Why Momentum Really Works" article, then it falls completely flat.
...
Turns out, this one-line change was all that's needed.
