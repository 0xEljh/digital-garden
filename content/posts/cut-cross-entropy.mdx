...
mathTooltips:
  - tex: "- z_{y_i}"
    tooltip: "Logit for the correct class $y_i$."
  - tex: "z_k"
    tooltip: "Logit for class $k$."
---

This is the source code that's referenced throughout this article. It might be helpful to have this open in another tab: <GithubPreview repo="apple/ml-cross-entropy">GitHub - Apple ML Cut Cross Entropy</GithubPreview>

## Memory-Efficient Cross-Entropy is a Game Changer

Cross Entropy is a ubiquitous loss function in deep learning, including the training of LLMs (next token prediction is a classification task). However, computing the cross entropy loss can be memory-intensive, as vocab sizes in LLMs can be enormous. As noted in <PaperPreview arxivId="2411.09009">Apple's CCE paper</PaperPreview>, cross-entropy loss can account for **40–90% of total GPU memory usage**.
...
Between the kernels, the `cce_lse_forward_kernel` is more interesting, so [lets explore its triton implementation](https://github.com/apple/ml-cross-entropy/blob/main/cut_cross_entropy/cce_lse_forward.py#L138).
...
> $$ and $$ \hat{S} C $$ or $$ \hat{S}^T E $$ with intermediate matrices $S$ and
> $$ \hat{S} $$ that do not fit into GPU memory and undergo a non-linear
> operation. We take a similar approach to the forward pass, recomputing the
> matrix $$ C^T E$$ implicitly in the GPU's shared memory. For the backward
> pass, we do not need to compute the normalization constant of the softmax,
> since $$ S = \text{softmax}(C^T E) = \exp(C^T E - \text{LSE}) $$. This allows
> us to reuse the global synchronization of the forward pass, and compute $S$
> efficiently in parallel.
> <br />
> We implement the second matrix multiplication in the main memory of the GPU,
> as a canonical blockwise implementation would require storing or synchronizing
> $S$. Algorithm 3 and Fig. 2c summarize the computation and access patterns. A
> naive implementation of this algorithm requires zero additional memory but is
> slow due to repeated global memory load and store operations. We use two
> techniques to improve the memory access pattern: gradient filtering and
> vocabulary sorting.
...
If you can recall chain-rule, you can see why $$\hat{S}$$ makes up the gradient of both the query and class embeddings.
It is the key intermediate product in this series of steps.
